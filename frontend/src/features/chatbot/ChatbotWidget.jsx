...
export default function ChatbotWidget({ enabled = true }) {
  ...
  const submitMessage = async (text = input) => {
    ...
    try {
      const result = await sendChatbotMessage(trimmed)
  ...
  return (
    <>
      {isOpen && (
        <section className="fixed bottom-24 right-4 z-40 flex max-h-[72vh] w-[min(390px,calc(100vw-32px))] flex-col overflow-hidden rounded-lg border border-ai-cyan/35 bg-[#070b14]/95 shadow-[0_18px_60px_rgba(0,0,0,0.45)] backdrop-blur md:right-6">
