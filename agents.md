...
18. **백그라운드 스냅샷 스케줄러 기동 가이드**:
   * Flask의 기동 방식(gunicorn, flask run 등)에 관계없이 백그라운드 포트폴리오 스냅샷 수집 스레드가 상시 구동되도록 `app.py` 전역 영역에 기동 로직을 배치하되, 중복 기동 방지 가드(`_ml_automation_started` 및 `_news_ingest_started`)를 적용하십시오.
   * 개발 및 로컬 테스트 편의를 위해 `.env`의 `PORTFOLIO_SNAPSHOT_RUN_ON_START=true` 설정 시 서버 로드 즉시 첫 1회 강제 스냅샷 수집을 실행하도록 지원하십시오.
