...
11. **커밋 메시지 한글화 규칙**: git 커밋 메시지는 Conventional Commit prefix(`docs:`, `refactor:`, `test:`, `fix:` 등)는 영문으로 유지하되, 콜론 뒤 설명은 반드시 한글로 작성하십시오. 예: `docs: AssetDetail 리팩토링 상태 기록`, `refactor: 종목 상세 공통 모델 분리`.
12. **통합 금융 차트 및 시세 API 어댑터 규칙**:
...
14. **실시간 호가 및 체결 API의 동적 폴백(Fallback) 보정**:
...
17. **Toss 실시간 환율 Live API 연동 및 폴백 규칙**:
...
20. **분산 락(Distributed Lock)과 토큰 캐시 DB화 규칙**:
...
21. **환경 변수 추가 시 .env.example 갱신 필수화**:
...
22. **무조건적 보안 하드캡 및 Fail-Fast 설계 규칙 (보안 필수)**:
...
23. **Supabase RLS 공용-개인 데이터 분리 격리 규칙 (DB 필수)**:
